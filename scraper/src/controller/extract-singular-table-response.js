...
function parseRBICircularDetails(html) {
...
const extract_page_data = async () => {
  try {
    ...
    const htmlcontent = await axios.get(
      `https://m.rbi.org.in//scripts/BS_CircularIndexDisplay.aspx?Id=12780`
    );

    // Parse the HTML
    const result = parseRBICircularDetails(htmlcontent.data);

    // Convert to formatted JSON
    const jsonResult = JSON.stringify(result, null, 2);

    // Save to file
    fs.writeFileSync("rbi_circular_details.json", jsonResult);

    console.log("Successfully converted HTML to JSON");
